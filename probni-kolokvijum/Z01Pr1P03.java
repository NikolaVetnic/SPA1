...
class SpisakPredmeta {
	...
	public void stampajPredmete(String m, int k) {
		
		System.out.println("Predmeti sa '" + m + "' u imenu i manje od " + k + " studenata: ");
		...
		for (int i = 0; i < br; i++) {
			...
			if (spisak[i].m.equals(m) && spisak[i].b < k) {
				...
				if (spisak[i].p.length() <= 0)
					System.out.print(" - za predmet nije definisan profesor.");
		...
		if (!nadjen) System.out.println("Ne postoje takvi predmeti.");
	}
