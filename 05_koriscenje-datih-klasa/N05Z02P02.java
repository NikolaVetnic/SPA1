 ...
 * b) sa početka reda opsluživanja uklanja stringove koji su kraći od 6
 *    znakova
 * c) ispisati sadržaj tako dobijenog reda u fajl koji precizira koris-
 *    nik
**/
...
class N05Z02P02 {
	...
	public static void ukloniKrace(Red<String> red) {
		
		if (!red.jePrazan()) {
			
			String s = red.prvi();
			
			while (!red.jePrazan() && s.length() < 6) {
	...
	public static void sacuvajRed(String f, Red<String> red) {
		
		Red<String> pom = red;
		
		while (!pom.jePrazan())
			Svetovid.out(f).println(pom.izbaciPrvi());
	...
	public static void main(String[] args) {
		...
		Red<String> red = ucitajRed("r1.txt");
		System.out.println(red);
		
		ukloniKrace(red);
		System.out.println(red);
		
		sacuvajRed("r1mod.txt", red);
	}
}
